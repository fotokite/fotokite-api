...
state = {"flight_state": None, "altitude_changed": False, "first_rotation": False, "second_rotation": False, "landing": False}

def handle_flight(data) -> None:
    logging.info("Flight Telemetry:\n%s", json.dumps(data, indent=2))
    flight_state = data.get("flight_state")
    state["flight_state"] = flight_state

    if flight_state != "Flying": return
    ...
    elif state["altitude_changed"] and state["first_rotation"] and not state["second_rotation"]:
    ...
    elif state["altitude_changed"] and state["first_rotation"] and state["second_rotation"] and not state["landing"]:
...
def demo_flight() -> None:
    ...
    system_thread = threading.Thread(
        ...
        args=(lambda data: logging.info("System Telemetry:\n%s", json.dumps(data, indent=2)),),
        daemon=True,
    )
    system_thread.start()
